...
TEST_CASE( "variable access map computation - empty system", "[access map]" ) {
  ...
  tchecker::variable_access_map_t map = tchecker::variable_access(model);
...
TEST_CASE( "variable access map computation - 1 process", "[access map]" ) {
  ...
  tchecker::process_id_t P = model.system().processes().key("P");
  tchecker::intvar_id_t i = model.system_integer_variables().id("i");
...
TEST_CASE( "variable access map computation - 2 processes, no shared variable", "[access map]" ) {
  std::string declarations =
  "system:access_map_2_processes_no_shared \n\
  event:a \n\
  \n\
  int:1:1:1:1:i \n\
  clock:1:x \n\
...
TEST_CASE( "variable access map computation - 3 processes, array, shared variables", "[access map]" ) {
  std::string declarations =
  "system:access_map_3_processes_shared \n\
  event:a \n\
  \n\
  int:1:1:1:1:i \n\
  int:3:0:1:0:t \n\
  ...
  tchecker::intvar_id_t i = model.system_integer_variables().id("i");
  tchecker::intvar_id_t t = model.system_integer_variables().id("t");
  ...
  SECTION( "clock variables ANY-accessed by process P3" ) {
    ...
    std::size_t y_size = model.system_clock_variables().info(y).size();
