...
class ts_t
    : public tchecker::ts::ts_t<STATE, TRANSITION, typename ZG::initial_iterator_t, typename ZG::outgoing_edges_iterator_t,
                                typename ZG::initial_iterator_value_t, typename ZG::outgoing_edges_iterator_value_t> {
public:
  /*!
   \brief Constructor
   \param model : a model to build ZG
   */
  template <class MODEL> explicit ts_t(MODEL & model) : _zg(model) {}
   ...
   */
  virtual tchecker::range_t<typename ZG::initial_iterator_t> initial() { return _zg.initial(); }

  /*!
   \brief Initialize state (see tchecker::ts::ts_t::initialize)
   */
  virtual enum tchecker::state_status_t initialize(STATE & s, TRANSITION & t, typename ZG::initial_iterator_value_t const & v)
  {
    return _zg.initialize(*s.vloc_ptr(), *s.intvars_valuation_ptr(), *s.zone_ptr(), v, t.src_invariant_container());
  }
   ...
   */
  virtual enum tchecker::state_status_t next(STATE & s, TRANSITION & t, typename ZG::outgoing_edges_iterator_value_t const & v)
  {
    return _zg.next(*s.vloc_ptr(), *s.intvars_valuation_ptr(), *s.zone_ptr(), v, t.src_invariant_container(),
   ...
   */
  inline constexpr ZG const & zg() const { return _zg; }
   ...
   */
  inline constexpr model_t const & model() const { return _zg.model(); }
