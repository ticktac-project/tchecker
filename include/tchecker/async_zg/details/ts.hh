...
class ts_t
    : public tchecker::ts::ts_t<STATE, TRANSITION, typename ASYNC_ZG::initial_iterator_t,
                                typename ASYNC_ZG::outgoing_edges_iterator_t, typename ASYNC_ZG::initial_iterator_value_t,
...
public:
   ...
   */
  template <class MODEL> explicit ts_t(MODEL & model) : _async_zg(model) {}
   ...
   */
  virtual tchecker::range_t<typename ASYNC_ZG::initial_iterator_t> initial() { return _async_zg.initial(); }

  /*!
   \brief Initialize state (see tchecker::ts::ts_t::initialize)
   */
  virtual enum tchecker::state_status_t initialize(STATE & s, TRANSITION & t,
                                                   typename ASYNC_ZG::initial_iterator_value_t const & v)
  {
    return _async_zg.initialize(*s.vloc_ptr(), *s.intvars_valuation_ptr(), *s.offset_zone_ptr(), *s.sync_zone_ptr(), v,
   ...
   */
  virtual enum tchecker::state_status_t next(STATE & s, TRANSITION & t,
                                             typename ASYNC_ZG::outgoing_edges_iterator_value_t const & v)
  {
    return _async_zg.next(*s.vloc_ptr(), *s.intvars_valuation_ptr(), *s.offset_zone_ptr(), *s.sync_zone_ptr(), v,
   ...
   */
  inline constexpr ASYNC_ZG const & async_zg() const { return _async_zg; }
   ...
   */
  inline constexpr model_t const & model() const { return _async_zg.model(); }
