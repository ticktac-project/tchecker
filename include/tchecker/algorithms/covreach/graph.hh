...
public:
   ...
   */
  template <class... SARGS> explicit node_t(SARGS &&... sargs) : STATE(std::forward<SARGS>(sargs)...), _protected(0) {}
   ...
   */
  bool is_protected() const { return (_protected == 1); }

  /*!
   \brief Make protected
   \post this node is protected
   */
  void make_protected() { _protected = 1; }

  /*!
   \brief Accessor
   \return node identifier
   */
  std::size_t identifier() const { return reinterpret_cast<std::size_t>(this); }
 ...
 */
template <class STATE, class EDGE_PTR> class allocation_size_t<tchecker::covreach::node_t<STATE, EDGE_PTR>> {
public:
 ...
 */
template <class NODE_PTR, class EDGE_PTR> class edge_t : public tchecker::graph::directed::edge_t<NODE_PTR, EDGE_PTR> {
public:
  /*!
   \brief Constructor
   ...
   */
  explicit edge_t(enum tchecker::covreach::edge_type_t edge_type = tchecker::covreach::ACTUAL_EDGE) : _edge_type(edge_type) {}
  ...
  /*!
   \brief Assignment operator
   */
  tchecker::covreach::edge_t<NODE_PTR, EDGE_PTR> & operator=(tchecker::covreach::edge_t<NODE_PTR, EDGE_PTR> const &) = default;
  ...
  /*!
   ...
   \return edge type
   */
  inline enum tchecker::covreach::edge_type_t edge_type() const { return _edge_type; }
   ...
   */
  inline void set_type(enum tchecker::covreach::edge_type_t edge_type) { _edge_type = edge_type; }
 ...
 */
template <class NODE_PTR, class EDGE_PTR> class allocation_size_t<tchecker::covreach::edge_t<NODE_PTR, EDGE_PTR>> {
public:
  template <class... ARGS> static constexpr std::size_t alloc_size(ARGS &&... args)
  {
 ...
 */
template <class STATE> using edge_t = tchecker::make_shared_t<edge_impl_t<STATE>>;

/*!
 \brief Type of pointers to edge
 */
template <class STATE> using edge_ptr_t = tchecker::intrusive_shared_ptr_t<edge_t<STATE>>;

/*!
 \brief Type of nodes
 */
template <class STATE> class node_impl_t : public tchecker::covreach::node_t<STATE, edge_ptr_t<STATE>> {
public:
...
/*!
 \brief Type of edges
 */
template <class STATE> class edge_impl_t : public tchecker::covreach::edge_t<node_ptr_t<STATE>, edge_ptr_t<STATE>> {
public:
 ...
 */
template <class TS> class graph_types_t {
public:
  /*!
   \brief Type of (shared) nodes
   */
 ...
 */
template <class STATE> class allocation_size_t<tchecker::covreach::details::node_impl_t<STATE>> {
public:
  template <class... ARGS> static constexpr std::size_t alloc_size(ARGS &&... args)
  {
 ...
 */
template <class STATE> class allocation_size_t<tchecker::covreach::details::edge_impl_t<STATE>> {
public:
  template <class... ARGS> static constexpr std::size_t alloc_size(ARGS &&... args)
  {
...
class graph_t
    : public tchecker::covreach::details::graph_types_t<TS>,
      ...
      private tchecker::graph::directed::graph_t<typename tchecker::covreach::details::graph_types_t<TS>::node_ptr_t,
...
public:
   ...
   */
  template <class... ARGS>
  graph_t(tchecker::gc_t & gc, std::tuple<ARGS...> && ts_alloc_args, std::size_t block_size, std::size_t table_size,
          node_to_key_t node_to_key, node_binary_predicate_t le_node)
      : tchecker::graph::cover::graph_t<node_ptr_t, key_t>(table_size, std::forward<node_to_key_t>(node_to_key),
   ...
   */
  template <class INSERTER> void covered_nodes(node_ptr_t const & n, INSERTER & ins) const
  {
   ...
   */
  inline enum tchecker::covreach::edge_type_t edge_type(edge_ptr_t const & edge) const { return edge->edge_type(); }

  /*!
   \brief Accessor
   \return the TS allocator of this graph
   */
  inline TS_ALLOCATOR & ts_allocator() { return _ts_allocator; }

  /*!
   \brief Accessor
   \return Number of nodes in this graph
   */
  std::size_t nodes_count() const { return tchecker::graph::cover::graph_t<node_ptr_t, key_t>::nodes_count(); }

  using const_iterator_t = typename tchecker::graph::cover::graph_t<node_ptr_t, key_t>::const_iterator_t;

  const_iterator_t begin() const { return tchecker::graph::cover::graph_t<node_ptr_t, key_t>::begin(); }

  const_iterator_t end() const { return tchecker::graph::cover::graph_t<node_ptr_t, key_t>::end(); }
