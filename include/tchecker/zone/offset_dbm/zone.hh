...
public:
  ...
  /*!
   ...
   \param zone : a zone
   \return 0 if this and zone are equal, a negative value if this is smalelr than zone w.r.t.t lexical ordering on the clock
   ...
   */
  static inline void destruct(tchecker::offset_dbm::zone_t * ptr) { ptr->~zone_t(); }
 ...
 */
inline std::size_t hash_value(tchecker::offset_dbm::zone_t const & zone) { return zone.hash(); }

/*!
 \brief Lexical ordering
 \param zone1 : first zone
 \param zone2 : second zone
 \return 0 if zone1 and zone2 are equal, a negative value if zone1 is smaller than zone2 w.r.t. lexical ordering, a positive
 value otherwise
 */
 ...
 */
template <> class allocation_size_t<tchecker::offset_dbm::zone_t> {
public:
  /*!
   \brief Accessor
   \param dim : dimension
 ...
 */
template <class... ARGS> tchecker::offset_dbm::zone_t * zone_allocate_and_construct(tchecker::clock_id_t dim, ARGS &&... args)
{
  if (dim < 1)
