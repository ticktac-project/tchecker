 ...
 */
template <class TS, class ALLOCATOR> class builder_t {
...
public:
  ...
  template <class... SARGS, class... TARGS>
  std::tuple<state_ptr_t, transition_ptr_t, tchecker::state_status_t>
  next_state(state_ptr_t & state, typename TS::outgoing_edges_iterator_value_t const & v, std::tuple<SARGS...> && sargs,
...
/*!
 ...
 \tparam ALLOCATOR : type of allocator (see tchecker::ts::allocator_t), should be garbage collected
 */
template <class TS, class ALLOCATOR> class builder_ok_t : private tchecker::ts::builder_t<TS, ALLOCATOR> {
public:
  /*!
   \brief Type of pointer to state
   */
   ...
   */
  template <class TS_ITERATOR> class iterator_t {
  public:
    /*!
     ...
     */
    iterator_t(TS_ITERATOR const & it, state_transition_computer_t computer) : _it(it), _computer(std::move(computer))
    {
      skip_bad();
    }
    ...
    /*!
     ...
     \param computer : state/transition computing function
     */
    iterator_t(TS_ITERATOR && it, state_transition_computer_t computer) : _it(std::move(it)), _computer(std::move(computer))
    {
      skip_bad();
    }
     ...
     */
    inline bool at_end() const { return _it.at_end(); }
  ...
  tchecker::range_t<tchecker::ts::builder_ok_t<TS, ALLOCATOR>::initial_iterator_t> initial()
  {
    ...
    return tchecker::make_range(tchecker::ts::builder_ok_t<TS, ALLOCATOR>::initial_iterator_t(initial_range.begin(), computer),
   ...
   */
  using outgoing_iterator_t = tchecker::ts::builder_ok_t<TS, ALLOCATOR>::iterator_t<typename TS::outgoing_edges_iterator_t>;
