...
public:
   ...
   */
  template <class... ARGS> static tchecker::make_shared_t<T, REFCOUNT, RESERVED> * allocate_and_construct(ARGS &&... args)
  {
   ...
   */
  inline constexpr std::size_t refcount(void) const { return *(refcount_addr()); }
 ...
 */
template <class T> class allocation_size_t<tchecker::make_shared_t<T>> {
public:
  /*!
   \brief Accessor
...
/*!
 ...
 Y.
 */
template <class T> class intrusive_shared_ptr_t {
public:
  /*!
   ...
   */
  intrusive_shared_ptr_t(T * t = nullptr) : _t(nullptr) { reset(t); }
   ...
   */
  intrusive_shared_ptr_t(tchecker::intrusive_shared_ptr_t<T> const & p) : _t(nullptr) { reset(p._t); }
   ...
   */
  ~intrusive_shared_ptr_t() { reset(nullptr); }
   ...
   */
  inline constexpr T * ptr() const { return _t; }
 ...
 */
template <class T> std::size_t hash_value(tchecker::intrusive_shared_ptr_t<T> const & p)
{
  boost::hash<T *> hasher;
  return hasher(p.ptr());
}
...
public:
  /*!
   ...
   \return Hash value for p
   */
  template <class T> inline std::size_t operator()(tchecker::intrusive_shared_ptr_t<T> const & p) const
  {
    return hash_value(p);
  }
};
...
public:
   ...
   */
  template <class T> std::size_t operator()(tchecker::intrusive_shared_ptr_t<T> const & p) const { return hash_value(*p); }
};
 ...
 */
template <class T> class hash<tchecker::intrusive_shared_ptr_t<T>> {
public:
  /*!
   \brief Hash function
   \param p : intrusive shared pointer
   ...
   */
  std::size_t operator()(tchecker::intrusive_shared_ptr_t<T> const & p) const { return tchecker::hash_value(p); }
};
