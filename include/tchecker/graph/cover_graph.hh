 ...
 */
template <class KEY, class NODE_PTR> using node_to_key_t = std::function<KEY(NODE_PTR const &)>;

/*!
 \brief Type of binary predicate on nodes
 \tparam NODE_PTR : type of pointers to node
 */
template <class NODE_PTR> using node_binary_predicate_t = std::function<bool(NODE_PTR const &, NODE_PTR const &)>;
 ...
 */
template <class NODE_PTR> class nodes_container_t {
public:
  /*!
   \brief Clear
   ...
   */
  tchecker::graph::cover::details::nodes_container_t<NODE_PTR>::const_iterator_t begin() const { return _vector.begin(); }

  /*!
   \brief Accessor
   \return constant past-the-end iterator
   */
  tchecker::graph::cover::details::nodes_container_t<NODE_PTR>::const_iterator_t end() const { return _vector.end(); }
...
private:
   ...
   */
  inline void position_in_table(tchecker::graph::cover::node_position_t position) { _position_in_table = position; }
   ...
   */
  inline bool is_stored() const { return ((_position_in_table != 0) || (_position_in_container != _NOT_A_POSITION)); }
 ...
 */
template <class NODE_PTR, class KEY = std::size_t> class graph_t {
...
public:
   ...
   */
  graph_t(std::size_t table_size, tchecker::graph::cover::node_to_key_t<KEY, NODE_PTR> node_to_key,
  ...
  /*!
   \brief Assignment operator
   */
  tchecker::graph::cover::graph_t<NODE_PTR, KEY> & operator=(tchecker::graph::cover::graph_t<NODE_PTR, KEY> const &) = default;
  ...
  tchecker::graph::cover::graph_t<NODE_PTR, KEY>::const_iterator_t begin() const
  {
    return tchecker::graph::cover::graph_t<NODE_PTR, KEY>::const_iterator_t(
        ...
        [](typename nodes_map_t::const_iterator const & it) { return std::make_tuple(it->begin(), it->end()); });
  }
