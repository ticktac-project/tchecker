...
/*!
\class synchronizable_standard_semantics_t
\brief Standard semantics: each transition in the zone graph consists of a delay
(if allowed) followed by a transition from the timed automaton, extended with
synchronizable check on zones: only zones that contain a synchronized valuation
are considered valid
*/
class synchronizable_standard_semantics_t final : public tchecker::refzg::semantics_t {
public:
  /*!
  \brief Destructor
  */
  virtual ~synchronizable_standard_semantics_t() = default;
  ...
  \post rdbm is the zone that only containts the zero valuation
  \return tchecker::STATE_OK if the resulting DBM is not empty. Otherwise,
  tchecker::STATE_CLOCKS_EMPTY_SPREAD if the zero valuation is not spread
  bounded,
  tchecker::STATE_CLOCKS_SRC_INVARIANT_VIOLATED if the zero valuation does not
  satisfy invariant,
  tchecker::STATE_ZONE_EMPTY_SYNC if the resulting DBM does not contain any
  ...
  */
  ...
  by assertion).
  \post rdbm has been delayed (only reference clocks which are allowed in
  src_delay_allowed), then the spread in rdbm has been bounded to spread, then
  intersected with src_invariant, then all reference clocks in sync_ref_clocks
  have been synchronized in rdbm, then rdbm has been intersected with guard,
  then dbm has been reset w.r.t clkreset, then intersected with tgt_invariant
  \return tchecker::STATE_OK if the resulting DBM is not empty. Otherwise,
  tchecker::STATE_CLOCKS_SRC_INVARIANT_VIOLATED if intersection with src_invariant
  result in an empty zone,
  tchecker::STATE_CLOCKS_EMPTY_SPREAD if bounding the spread result in the empty
  zone,
  tchecker::STATE_CLOCKS_SYNC_EMPTY if synchronization of reference clocks in
  sync_ref_clocks yield an empty zone,
  tchecker::STATE_CLOCKS_GUARD_VIOLATED if intersection with guard result in an
...
/*!
\class synchronizable_elapsed_semantics_t
\brief elapsed semantics: each transition in the zone graph consists of a
transition from the automaton, followed by a delay (if allowed), extended with
synchronizable check on zones: only zones that contain a synchronized valuation
are considered valid
*/
class synchronizable_elapsed_semantics_t final : public tchecker::refzg::semantics_t {
public:
  /*!
  \brief Destructor
  */
  virtual ~synchronizable_elapsed_semantics_t() = default;
  ...
  \post rdbm is the zone that contains all the time successors of the zero
  valuation (for reference clocks that are allowed to delay in delay_allowed),
  that satisfy invariant, and with a bounded spread
  \return tchecker::STATE_OK if the resulting DBM is not empty. Otherwise,
  tchecker::STATE_CLOCKS_SRC_INVARIANT_VIOLATED if the (time successors of the)
  zero valuation does not satisfy invariant,
  tchecker::STATE_CLOCKS_EMPTY_SPREAD if spread bounding results in the empty
...
enum semantics_type_t {
  ...
  SYNC_STANDARD_SEMANTICS, /*!< see tchecker::refzg::synchronizable_standard_semantics_t */
  SYNC_ELAPSED_SEMANTICS,  /*!< see tchecker::refzg::synchronizable_elapsed_semantics_t */
};
