...
{
  return ((type == tchecker::EXPR_TYPE_INTTERM) || (type == tchecker::EXPR_TYPE_INTVAR) ||
          (type == tchecker::EXPR_TYPE_INTLVALUE) || (type == tchecker::EXPR_TYPE_LOCALINTVAR) ||
...
enum tchecker::expression_type_t type_binary_predicate(enum tchecker::expression_type_t left,
...
enum tchecker::expression_type_t type_binary(enum tchecker::binary_operator_t op, enum tchecker::expression_type_t left,
                                             enum tchecker::expression_type_t right)
{
  switch (op) {
  case tchecker::EXPR_OP_LAND:
    return type_land(left, right);
  case tchecker::EXPR_OP_LT:
    return type_lt(left, right);
  case tchecker::EXPR_OP_LE:
    return type_le(left, right);
  case tchecker::EXPR_OP_EQ:
    return type_eq(left, right);
  case tchecker::EXPR_OP_NEQ:
    return type_neq(left, right);
  case tchecker::EXPR_OP_GE:
    return type_ge(left, right);
  case tchecker::EXPR_OP_GT:
    return type_gt(left, right);
  case tchecker::EXPR_OP_MINUS:
    return type_minus(left, right);
  case tchecker::EXPR_OP_PLUS:
    return type_plus(left, right);
  case tchecker::EXPR_OP_TIMES:
    return type_times(left, right);
  case tchecker::EXPR_OP_DIV:
    return type_div(left, right);
  case tchecker::EXPR_OP_MOD:
    return type_mod(left, right);
  default:
...
enum tchecker::expression_type_t type_unary(enum tchecker::unary_operator_t op, enum tchecker::expression_type_t expr_type)
{
  ...
  case tchecker::EXPR_OP_NEG:
    return type_neg(expr_type);
  case tchecker::EXPR_OP_LNOT:
    return type_lnot(expr_type);
  default:
    throw std::runtime_error("incomplete switch statement");
  }
}

enum tchecker::expression_type_t type_ite(enum tchecker::expression_type_t cond, enum tchecker::expression_type_t then_value,
