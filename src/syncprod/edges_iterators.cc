...
namespace tchecker {
  ...
  namespace syncprod {
                                ...
                                tchecker::intrusive_shared_ptr_t<tchecker::shared_vloc_t const> const & vloc)
    {
      ...
      auto loc_to_async_edges = [&system] (tchecker::loc_id_t loc) {
        return system.asynchronous_outgoing_edges(loc);
      };
      ...
      // join iterator
      auto get_sub_range = [] (tchecker::syncprod::vloc_asynchronous_edges_const_iterator_t const & it) {
        return it->iterators();
      };
    ...
    \return true if every process involved in sync has a corresponding transition from vloc according to loc_edges_maps, false othwerwise
    */
    static inline bool enabled(tchecker::system::synchronization_t const & sync,
                               ...
                               tchecker::system::loc_edges_maps_t const & loc_edges_maps)
    {
      for (tchecker::system::sync_constraint_t const & constr : sync.synchronization_constraints()) {
        if (constr.strength() == tchecker::SYNC_STRONG &&
            ! loc_edges_maps.event(vloc[constr.pid()], constr.event_id()))
          return false;
      }
      return true;
    }
    ...
    bool vloc_synchronized_edges_iterator_t::operator==
    (tchecker::syncprod::vloc_synchronized_edges_iterator_t const & it) const
    {
      return ((*_vloc == *it._vloc) &&
              (&_loc_edges_maps.get() == &it._loc_edges_maps.get()) &&
    ...
    void tchecker::syncprod::vloc_synchronized_edges_iterator_t::advance_while_empty_cartesian_product()
    {
      ...
      for (auto const & constr : constraints) {
        auto edges = _loc_edges_maps.get().edges((*_vloc)[constr.pid()], constr.event_id());
                                ...
                                tchecker::intrusive_shared_ptr_t<tchecker::shared_vloc_t const> const & vloc)
    {
      ...
      tchecker::syncprod::vloc_synchronized_edges_iterator_t begin(vloc,
                                                                   system.outgoing_edges_maps(),
                                                                   syncs.begin(),
                                                                   syncs.end());
      tchecker::syncprod::vloc_synchronized_edges_iterator_t end(vloc,
                                                                 system.outgoing_edges_maps(),
                                                                 syncs.end(),
                                                                 syncs.end());
    ...
    (tchecker::syncprod::vloc_synchronized_edges_iterator_t::edges_iterator_t const & it)
    ...
    {
      return ((_async_edge == it._async_edge) &&
              (_async_at_end == it._async_at_end) &&
              (_sync_it == it._sync_it));
    }
    ...
    vloc_edges_iterator_t::vloc_edges_iterator_t
    (tchecker::syncprod::vloc_synchronized_edges_iterator_t const & sync_it,
