...
namespace tchecker {
  ...
  namespace syncprod {
    ...
    tchecker::syncprod::system_t::asynchronous_edges_collection_t const
    tchecker::syncprod::system_t::_empty_async_edges;
    
    
    system_t::system_t(tchecker::parsing::system_declaration_t const & sysdecl)
    : tchecker::system::system_t(sysdecl)
    {
      extract_asynchronous_edges();
    }
    
    
    system_t::system_t(tchecker::system::system_t const & system)
    : tchecker::system::system_t(system)
    {
      extract_asynchronous_edges();
    }
    ...
    public:
       ...
       */
      synchronizer_t(tchecker::syncprod::system_t const & system,
                     std::string const & separator,
                     std::string const & process_name)
      : _system(system),
      _separator(separator),
      _process_name(process_name),
      _product(_system.name(), _system.attributes()),
      _salloc(1000000, 1000000, _system.processes_count()),
      _talloc(1000000, 1000000, _system.processes_count())
      {
        integer_variables();
        clock_variables();
        process();
       ...
       */
      tchecker::system::system_t synchronized_product() const
      {
        return _product;
      }
    private:
      ...
      std::string namify(transition_ptr_t const & transition)
      {
        ...
        for (auto it = begin; it != end; ++it) {
          if (it != begin)
            name += _separator;
          name += _system.process_name(_system.edge(*it)->pid()) + _separator +
      ...
      {
        for (tchecker::intvar_id_t id = 0; id < _system.intvars_count(); ++id) {
          tchecker::intvar_info_t const & info = _system.integer_variables().info(id);
          _product.add_intvar(_system.intvar_name(id), info.size(), info.min(), info.max(),
                              info.initial_value(), _system.intvar_attributes(id));
        }
      }
      ...
      {
        for (tchecker::clock_id_t id = 0; id < _system.clocks_count(); ++id) {
      ...
      void process()
      {
        _product.add_process(_process_name);
      }
       ...
       */
      void initial_states(tchecker::syncprod::syncprod_t & sp,
                          std::vector<std::tuple<state_ptr_t, transition_ptr_t>> & v)
      {
       ...
       */
      void next_states(tchecker::syncprod::syncprod_t & sp,
                       state_ptr_t const & state,
                       std::vector<std::tuple<state_ptr_t, transition_ptr_t>> & v)
      {
