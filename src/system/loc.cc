...
namespace tchecker {
  ...
  namespace system {
    ...
    loc_t::loc_t(tchecker::process_id_t pid,
    ...
    locs_t::locs_t(tchecker::system::locs_t const & locs)
    {
      add_locations(locs);
    }
    ...
    void locs_t::add_location(tchecker::process_id_t pid,
                              std::string const & name,
                              tchecker::system::attributes_t const & attr)
    {
    ...
    tchecker::system::loc_const_shared_ptr_t locs_t::location(tchecker::process_id_t pid,
                                                              std::string const & name)
    {
      if (pid >= _locs_index.size())
    ...
    locs_t::const_iterator_t::const_iterator_t
    (std::vector<tchecker::system::loc_shared_ptr_t>::const_iterator const & it)
    ...
    tchecker::range_t<tchecker::system::locs_t::const_iterator_t>
    locs_t::initial_locations(tchecker::process_id_t pid) const
    {
      if (pid >= _initial_locs.size())
    ...
    bool locs_t::is_location(tchecker::loc_id_t id) const
    {
      return (id < locations_count());
    }
