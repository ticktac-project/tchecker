...
#define DBM2(i,j)         dbm2[(i)*dim+(j)]
    ...
    void extra_m(tchecker::dbm::db_t * dbm, tchecker::clock_id_t dim, tchecker::dbm::db_t const * m)
    {
      ...
      for (tchecker::clock_id_t j = 1; j < dim; ++j) {
        ...
        if (DBM(0,j) == tchecker::dbm::LE_ZERO)
          continue;
        tchecker::dbm::db_t c0j = tchecker::dbm::value(DBM(0,j));
        if (-c0j > m[j]) {
      ...
      for (tchecker::clock_id_t i = 1; i < dim; ++i) {
        ...
        for (tchecker::clock_id_t j = 0; j < dim; ++j) {
          ...
          tchecker::dbm::db_t cij = tchecker::dbm::value(DBM(i,j));
          if (cij > m[i]) {
            DBM(i,j) = tchecker::dbm::LT_INFINITY;
            modified = true;
          }
    ...
    void extra_m_plus(tchecker::dbm::db_t * dbm, tchecker::clock_id_t dim, tchecker::dbm::db_t const * m)
    {
      ...
      for (tchecker::clock_id_t i = 1; i < dim; ++i) {
        ...
        tchecker::dbm::db_t c0i = tchecker::dbm::value(DBM(0,i));
    ...
    bool is_alu_le(tchecker::dbm::db_t const * dbm1, tchecker::dbm::db_t const * dbm2, tchecker::clock_id_t dim, tchecker::dbm::db_t const * l, tchecker::dbm::db_t const * u)
    {
      assert(dbm1 != nullptr);
      assert(dbm2 != nullptr);
      assert(dim >= 1);
    ...
    bool is_am_le(tchecker::dbm::db_t const * dbm1, tchecker::dbm::db_t const * dbm2, tchecker::clock_id_t dim, tchecker::dbm::db_t const * m)
    {
    ...
    std::ostream & output(std::ostream & os, tchecker::dbm::db_t const * dbm, tchecker::clock_id_t dim, std::function<std::string(tchecker::clock_id_t)> clock_name)
    {
      os << "(";
      ...
      for (tchecker::clock_id_t i = 1; i < dim; ++i) {
        for (tchecker::clock_id_t j = i+1; j < dim; ++j) {
          ...
          if (tchecker::dbm::sum(cij, cji) == tchecker::dbm::LE_ZERO) {
            ...
            tchecker::dbm::db_t vij = tchecker::dbm::value(cij);
            if (vij > 0)
